...
| T05 | FastAPI Safety Service endpoints | API, health-contract, request-metadata, classification-contract, interface-type, request-identity, text-utility, operation-registry, and reason-message tests call `/health`, `/v1/requests/canonicalize`, `/v1/requests/classify`, and `/v1/policy/evaluate` with expected schemas, centralized health response payload, centralized workspace/agent metadata mapping, stable fingerprints, centralized classifier confidence/reasons, centralized token parsing, centralized operation inference, and centralized policy reason text. | Bestanden | 2026-05-31 |
...
| T20 | Slack development channel | Slack adapter, Slack-contract, request-identity, payload-field, and secret-redaction tests use a mocked Slack API to confirm inbound request normalization, nested event object validation, centralized Slack source labels and adapter validation messages, stable request IDs, direct shared helper usage at the adapter boundary, outbound response formatting, and token redaction. | Bestanden | 2026-05-31 |
...
| T23 | GitHub adapter | GitHub adapter, GitHub-contract, connection-settings, execution-gate-message, direct operation registry, URL utility, HTTP-method, and adapter-result tests use a mocked GitHub API, resolve `git_ai-artist_codex_token` through the shared connection boundary, call shared operation and URL path validation directly, reject unsafe API paths and non-write methods before token reads, centralize required-envelope and GitHub adapter validation errors, and confirm the token is read only by the adapter, not agents. | Bestanden | 2026-05-31 |
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, health-contract, request-metadata, classification-contract, sub-agent output-constructor, direct runtime interface-type imports, registry-driven connection settings/runtime env, canonical hashing/version tags/HMAC signatures/security-review serialization, direct source-ingestion hash/version and URL-domain boundaries, source-registry missing-row message contracts and optional lookup, execution-gate failure and required-envelope message contracts, request identity/trace IDs, runtime IDs, mapping utilities, cache request-kind/operation constants, cache/policy/execution reason messages, sub-agent statuses, mock-agent contracts including orchestration output/error/synthesis/telemetry text, Knowledge Agent contracts and contextual snippets, ComfyUI contracts including response validation messages, review statuses, critic rubric vocabulary, Slack and GitHub contract and boundary helper usage, shared secret detection/redaction and file-scanning boundaries, direct gated-adapter operation constants, publishing operation/status boundaries, observability constants and redacted audit mappings, text utilities, Markdown heading utilities, direct numeric utility use at rubric boundaries, time utilities with direct-wrapper guard, payload fields/nested objects, response fields including first-choice message parsing, URL utilities with direct connector-boundary usage, HTTP method normalization, operation registry, direct model coercion at adapter/domain boundaries, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
