...
| T18 | Image provenance | Artifact, review-status, model-coercion, canonical-hash, payload-field, response-field, ComfyUI-contract, and time-utility tests verify prompt hash, workflow hash, model, seed, source refs, storage uri, centralized review status, centralized generated-image URI construction, stable canonical JSON digests, response image shape validation, and direct shared UTC timestamps are stored for every image. | Bestanden | 2026-05-31 |
| T19 | Critic/Curator rubrics | Critic-rubric, review-status, text-utility, numeric-utility, and model-coercion tests score sample images or mocked image metadata with shared rubric category/decision vocabulary, shared label/token normalization, shared review-status checks, and shared score clamping/averages, returning structured critique, pass/fail, and improvement notes. | Bestanden | 2026-05-31 |
...
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, direct runtime interface-type imports, registry-driven connection settings/runtime env, canonical hashing/version tags/HMAC signatures/security-review serialization, direct source-ingestion hash/version and URL-domain boundaries, request identity/trace IDs, runtime IDs, mapping utilities, cache request-kind/operation constants, cache/policy/execution reason messages, sub-agent statuses, mock-agent contracts, Knowledge Agent contracts, ComfyUI contracts, review statuses, critic rubric vocabulary, Slack boundary helper usage, direct OpenClaw secret-redaction usage, publishing statuses, observability constants and redacted audit mappings, text utilities, numeric utilities, time utilities with direct-wrapper guard, payload fields/nested objects, response fields including first-choice message parsing, URL utilities with direct connector-boundary usage, HTTP method normalization, operation registry, model coercion at adapter/domain boundaries, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
