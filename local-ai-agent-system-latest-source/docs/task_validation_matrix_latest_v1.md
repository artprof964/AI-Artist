...
| T05 | FastAPI Safety Service endpoints | API, request-identity, text-utility, and operation-registry tests call `/health`, `/v1/requests/canonicalize`, `/v1/requests/classify`, and `/v1/policy/evaluate` with expected schemas, stable fingerprints, centralized token parsing, and centralized operation inference. | Bestanden | 2026-05-31 |
...
| T09 | Provider-neutral LLM API configuration | Smoke, connection-settings, and response-field tests load DeepSeek model env vars through shared runtime env resolution, call the OpenAI-compatible LLM API with a redacted request, and record response id/model/content from dict or SDK-object responses without logging secrets. | Bestanden | 2026-05-31 |
...
| T12 | Mock sub-agents | Orchestration and numeric-utility tests route one request through all mock agents, collect structured outputs, and synthesize status/confidence through shared rounded-average helpers. | Bestanden | 2026-05-31 |
...
| T19 | Critic/Curator rubrics | Rubric, text-utility, numeric-utility, and model-coercion tests score sample images or mocked image metadata with shared label/token normalization and shared score clamping/averages, returning structured critique, pass/fail, and improvement notes. | Bestanden | 2026-05-31 |
...
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, connection settings/runtime env, canonical hashing/version tags, request identity/trace IDs, text utilities, numeric utilities, time utilities, payload fields/nested objects, response fields, URL utilities, operation registry, model coercion, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
