...
| T16 | Knowledge Agent retrieval | Retrieval and text-utility tests ingest sample source data, tokenize/embed it through shared helpers, query Qdrant, return source-cited results through the Knowledge Agent, and exclude unapproved sources. | Bestanden | 2026-05-31 |
...
| T19 | Critic/Curator rubrics | Rubric, text-utility, and model-coercion tests score sample images or mocked image metadata with shared label/token normalization and return structured critique, pass/fail, and improvement notes. | Bestanden | 2026-05-31 |
...
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, connection settings/runtime env, canonical hashing/version tags, request identity/trace IDs, text utilities, time utilities, payload fields/nested objects, response fields, URL utilities, operation registry, model coercion, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
