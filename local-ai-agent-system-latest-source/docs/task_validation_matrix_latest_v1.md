...
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, direct runtime interface-type imports, registry-driven connection settings/runtime env, canonical hashing/version tags/HMAC signatures/security-review serialization, direct source-ingestion hash/version and URL-domain boundaries, source-registry missing-row message contracts, execution-gate failure message contracts, request identity/trace IDs, runtime IDs, mapping utilities, cache request-kind/operation constants, cache/policy/execution reason messages, sub-agent statuses, mock-agent contracts, Knowledge Agent contracts, ComfyUI contracts, review statuses, critic rubric vocabulary, Slack boundary helper usage, shared secret detection/redaction boundaries, publishing statuses, observability constants and redacted audit mappings, text utilities, direct numeric utility use at rubric boundaries, time utilities with direct-wrapper guard, payload fields/nested objects, response fields including first-choice message parsing, URL utilities with direct connector-boundary usage, HTTP method normalization, operation registry, direct model coercion at adapter/domain boundaries, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
...
| T27 | Security review | Shared detection/redaction tests and security checklist tests scan logs, prompts, memory, audit payloads, artifacts, and canonical JSON serialization paths for secret patterns, assignment-style leaks, and policy bypasses. | Bestanden | 2026-05-31 |
