...
| T05 | FastAPI Safety Service endpoints | API, interface-type, request-identity, text-utility, operation-registry, and reason-message tests call `/health`, `/v1/requests/canonicalize`, `/v1/requests/classify`, and `/v1/policy/evaluate` with expected schemas, stable fingerprints, centralized token parsing, centralized operation inference, and centralized policy reason text. | Bestanden | 2026-05-31 |
...
| T12 | Mock sub-agents | Orchestration, mock-agent-contract, sub-agent status, review-status, and numeric-utility tests route one request through all mock agents, collect structured outputs, and synthesize status/confidence through shared agent/artifact vocabulary, status aggregation, and rounded-average helpers. | Bestanden | 2026-05-31 |
...
| T14 | Source freshness check | Freshness, source-registry-contract, and time-utility tests increment `source_data_registry.change_seq`, verify UTC source timestamps, prove stale cached responses are blocked, and guard shared missing-row message formatting for source-key/source-id lookup failures. | Bestanden | 2026-05-31 |
...
| T16 | Knowledge Agent retrieval | Retrieval, knowledge-contract, text-utility, and numeric-utility tests ingest sample source data, tokenize/embed it through shared helpers, query Qdrant with shared vector similarity, return source-cited results through centralized Knowledge Agent output contracts, and exclude unapproved sources. | Bestanden | 2026-05-31 |
| T17 | ComfyUI adapter behind execution gate | Shared model-coercion, operation, execution-gate, ComfyUI-contract, and time-utility tests plus adapter tests prove image generation fails without a non-expired valid execution envelope, uses direct shared UTC expiry checks, follows centralized generated-image URI conventions, and succeeds with a mocked approved envelope. | Bestanden | 2026-05-31 |
...
| T19 | Critic/Curator rubrics | Critic-rubric, review-status, text-utility, numeric-utility, and direct model-coercion tests score sample images or mocked image metadata with shared rubric category/decision vocabulary, shared label/token normalization, shared review-status checks, and shared score clamping/averages, returning structured critique, pass/fail, and improvement notes. | Bestanden | 2026-05-31 |
...
| T21 | Source ingestion | Ingestion, canonical-hash, and URL utility tests import approved sample sources, store snapshots, record source registry rows, call shared hash/version and URL-domain helpers directly, create stable source version tags, and reject disallowed or non-http source domains. | Bestanden | 2026-05-31 |
| T22 | Publishing Agent with human approval | Shared operation, execution-gate, canonical ID, response-field, publishing-status, side-effect audit, and publishing tests prove external publishing remains blocked until human approval is attached to the execution envelope and audit status is read through shared response/status boundaries. | Bestanden | 2026-05-31 |
| T23 | GitHub adapter | GitHub adapter, connection-settings, operation registry, URL utility, HTTP-method, and adapter-result tests use a mocked GitHub API, resolve `git_ai-artist_codex_token` through the shared connection boundary, call shared URL path validation directly, reject unsafe API paths and non-write methods before token reads, and confirm the token is read only by the adapter, not agents. | Bestanden | 2026-05-31 |
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, direct runtime interface-type imports, registry-driven connection settings/runtime env, canonical hashing/version tags/HMAC signatures/security-review serialization, direct source-ingestion hash/version and URL-domain boundaries, source-registry missing-row message contracts, request identity/trace IDs, runtime IDs, mapping utilities, cache request-kind/operation constants, cache/policy/execution reason messages, sub-agent statuses, mock-agent contracts, Knowledge Agent contracts, ComfyUI contracts, review statuses, critic rubric vocabulary, Slack boundary helper usage, direct OpenClaw secret-redaction usage, publishing statuses, observability constants and redacted audit mappings, text utilities, numeric utilities, time utilities with direct-wrapper guard, payload fields/nested objects, response fields including first-choice message parsing, URL utilities with direct connector-boundary usage, HTTP method normalization, operation registry, direct model coercion at adapter/domain boundaries, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
...
| T26 | Observability | Telemetry, audit-redaction, and observability-constant tests verify trace ids, metrics, structured logs, stage constants, log-level constants, and shared redacted mapping helpers are used for request, policy, cache, orchestration, and tool stages. | Bestanden | 2026-05-31 |
