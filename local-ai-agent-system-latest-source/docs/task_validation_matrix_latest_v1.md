...
| T05 | FastAPI Safety Service endpoints | API, health-contract, interface-type, request-identity, text-utility, operation-registry, and reason-message tests call `/health`, `/v1/requests/canonicalize`, `/v1/requests/classify`, and `/v1/policy/evaluate` with expected schemas, centralized health response payload, stable fingerprints, centralized token parsing, centralized operation inference, and centralized policy reason text. | Bestanden | 2026-05-31 |
