...
| T23 | GitHub adapter | GitHub adapter, URL utility, and adapter-result tests use a mocked GitHub API, reject unsafe API paths before token reads, and confirm `git_ai-artist_codex_token` is read only by the adapter, not agents. | Bestanden | 2026-05-31 |
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, canonical hashing, request identity, payload fields, URL utilities, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
