...
| T24 | Unit tests for safety service and policies | CI test suite runs canonicalizer, connection settings/runtime env, canonical hashing/version tags/HMAC signatures/security-review serialization, request identity/trace IDs, text utilities, numeric utilities, time utilities with direct-wrapper guard, payload fields/nested objects, response fields, URL utilities, operation registry, model coercion, classifier, OPA, cache, freshness, and audit unit tests with `--cov-fail-under=90` over the safety service modules. | Bestanden | 2026-05-31 |
