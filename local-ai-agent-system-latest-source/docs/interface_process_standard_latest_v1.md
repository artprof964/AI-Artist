...
61a. Gated-adapter and Publishing Agent tests use `tests/gated_adapter_helpers.py` for adapter-specific request IDs, targets, operation defaults, payload defaults, request construction, Publishing Agent request construction, approved/unapproved execution-envelope setup, adapter/agent/client harness setup, and deterministic fake-client responses.
...
64a. Slack adapter tests use `tests/slack_adapter_helpers.py` for adapter/client harness setup, deterministic Slack clients, inbound event payloads, event identity values, timestamps, and normalized text.
...
68a. Safety Service unit, observability, and request-metadata tests use `tests/service_request_helpers.py` for standard `CanonicalizeRequest` and `ClassifyRequest` setup.
...
- Slack adapter tests use `tests/slack_adapter_helpers.py` instead of
  constructing `SlackAdapter` directly or defining local Slack client doubles,
...
- Cache, source freshness, observability, policy-contract, Safety Service unit,
  and future policy-path tests use `tests/policy_request_helpers.py` instead
  of constructing standard `PolicyEvaluateRequest` fixtures directly.
...
- Safety Service unit, observability, request-metadata, and future service-boundary tests use
  `tests/service_request_helpers.py` instead of constructing
  `CanonicalizeRequest` or `ClassifyRequest` directly.
...
- Publishing Agent tests that need local publisher clients, secret-echo publisher
  ...
  envelopes use `tests/gated_adapter_helpers.py` instead of constructing
  `PublishingAgent`, `LocalPublishingClient`, `PublishingAgentRequest`, or local
  publishing envelope helpers directly.
...
- Gated-adapter and Publishing Agent tests that need deterministic fake
  ...
  classes in adapter-specific test modules.
