...
52. GitHub adapter action labels, validation messages, token-purpose text, token-required message routing, and explicit/env token lookup use `backend/github_contracts.py`, `backend/connection_settings.py`, and `backend/adapter_secrets.py`; standard adapter setting-name lookup is derived from the connection registry.
