...
Test path/source helpers: shared across repo-wide guard tests and checked-in source inspection tests for connection settings, canonical hashing, classification contracts, health contracts, request metadata, mapping utilities, model coercion, runtime IDs, Safety Service, security review, source ingestion, ComfyUI, Publishing, Slack, HTTP method, GitHub contract, production readiness, tree shape, time utilities, shell commands, and repo path contracts
...
test path helper validation: adapter/connector contract checks plus existing guard tests passed; migrated checked-in backend/source inspections share test path/source helpers
