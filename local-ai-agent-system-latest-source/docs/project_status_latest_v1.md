...
Test project-root resolution, checked-in project text reads, backend source reads, test source reads, repo-wide test-module source iteration, Safety Service test-client setup, Safety Service canonicalize/classify/observability/default-scope request setup, approved cache-entry setup, connection env setup, human-approval setup, image provenance store/payload/record setup, Knowledge Agent harness setup, LLM smoke client setup, OpenClaw hook and observability recording setup, policy-request construction, policy-response construction, request metadata construction, secret payload setup, Slack adapter harness setup, source-registry and source-ingestion harness setup, SubAgentOutput raw payload setup, policy-path execution-envelope construction, gated-adapter request/envelope/client/harness defaults, Publishing Agent request/client/harness setup, and OpenClaw tool-call request construction: centralized in tests/path_helpers.py, tests/safety_service_client_helpers.py, tests/service_request_helpers.py, tests/cache_entry_helpers.py, tests/connection_env_helpers.py, tests/human_approval_helpers.py, tests/image_provenance_helpers.py, tests/knowledge_agent_helpers.py, tests/llm_api_smoke_helpers.py, tests/openclaw_hook_helpers.py, tests/policy_request_helpers.py, tests/policy_response_helpers.py, tests/request_metadata_helpers.py, tests/secret_test_helpers.py, tests/slack_adapter_helpers.py, tests/source_registry_helpers.py, tests/subagent_output_helpers.py, tests/execution_envelope_helpers.py, tests/gated_adapter_helpers.py, and tests/tool_call_helpers.py
...
Test path/source, Safety Service client, Safety Service request, cache-entry, connection-env, human-approval, image provenance, Knowledge Agent harness, policy-request, policy-response, request-metadata, secret-payload, Slack adapter harness, source-registry/source-ingestion harness, SubAgentOutput payload, execution-envelope, gated-adapter request/envelope/client/harness, Publishing Agent request/client/harness, OpenClaw hook recording adapter, and tool-call helpers: shared across repo-wide guard tests, filesystem/process fixture tests, checked-in source inspection tests, endpoint/audit TestClient setup, Safety Service canonicalize/classify/observability/default-scope request setup, cache-path fixture setup, connection setup, envelope approval setup, image provenance store/payload/record setup, Knowledge Agent vector store/embedding/source/vector-point setup, redaction/security-review fixtures, Slack adapter/client/event fixture setup, policy-path request and response setup, request metadata setup, source registry fixture setup, source ingestion service/snapshot repository/sample candidate setup, raw SubAgentOutput schema payload setup, policy-path execution-envelope setup, gated-adapter request-id/target/operation/payload/request-construction defaults, Publishing Agent request/client/harness setup, deterministic fake-client responses, ComfyUI and Publishing adapter/client setup, GitHub token/env adapter setup, secret-echo publishing responses, OpenClaw hook and observability recording clients/adapters/event labels, and OpenClaw tool-call request setup for connection settings, connection env-access checks, canonical hashing, classification contracts, health contracts, request metadata, mapping utilities, model coercion, runtime IDs, Safety Service, security review, source ingestion, source freshness, ComfyUI, Publishing, publishing status, Slack, HTTP method, GitHub adapter, GitHub contract, policy contracts, image provenance, Critic/Curator, Knowledge Agent, mock sub-agents, sub-agent output/status, execution gate, interface types, LLM API smoke, observability, OpenClaw hook, reason messages, review statuses, response cache, production readiness, tree shape, time utilities, shell commands, OPA policy, PostgreSQL migration, OpenClaw workspace, file scanning, and repo path contracts
...
SubAgentOutput payload helper validation: 13 focused tests passed; schema-boundary tests share raw valid SubAgentOutput payload construction through tests/subagent_output_helpers.py and guard against local payload builders
...
Slack adapter harness validation: 41 focused connection-path tests passed; Slack adapter tests share adapter/client setup, deterministic Slack client doubles, inbound event payloads, event identity values, timestamps, and normalized text through tests/slack_adapter_helpers.py, and guard against direct SlackAdapter construction in tests
...
source registry and ingestion harness validation: 27 focused source tests passed; source freshness and source ingestion tests share SourceFreshnessRegistry, SourceIngestionService, snapshot repository, approved-domain, and sample-candidate setup through tests/source_registry_helpers.py, source-freshness tests call shared policy-request and cache-entry helpers directly, and guards prevent direct local constructor setup or local policy/cache wrapper functions
...
policy-request helper validation: 49 focused policy-path tests passed; cache, source freshness, observability, policy-contract, Safety Service unit, and OpenClaw hook tests share policy request and freshness helper setup, response-cache tests call the shared policy request helper directly without local base-policy-request wrappers, and policy-contract tests guard against direct PolicyEvaluateRequest imports
...
OpenClaw hook helper validation: 12 focused tests passed, 1 warning; OpenClaw safety hook and observability tests share recording Safety Service client, recording adapter, mock orchestration adapter, and event labels through tests/openclaw_hook_helpers.py
...
gated-adapter envelope helper validation: 54 focused tests passed; ComfyUI, Publishing, and GitHub adapter tests share adapter-specific request IDs, targets, operation defaults, approved/unapproved envelope setup, and guard against adapter-local envelope wrapper functions
gated-adapter request helper validation: 54 focused tests passed; ComfyUI, Publishing, and GitHub adapter tests share standard prompt/workflow/path/payload defaults and request construction, and guard against direct adapter request-model construction
gated-adapter client/harness helper validation: 58 focused gated-adapter tests passed; ComfyUI, Publishing, and GitHub adapter tests share deterministic fake clients and adapter/client harness setup through tests/gated_adapter_helpers.py and guard against direct adapter construction
Publishing Agent helper validation: 69 focused gated publishing and policy-contract tests passed; Publishing Agent tests share local publisher, secret-echo publisher, publish request, target, payload, and approved/unapproved envelope setup through tests/gated_adapter_helpers.py and guard against direct PublishingAgent/LocalPublishingClient/PublishingAgentRequest imports and local envelope builders
...
request metadata helper validation: 25 focused tests passed; request metadata, Safety Service unit, and observability tests share RequestMetadata construction and guard against direct local constructor setup
safety-service client helper validation: 27 focused tests passed, 1 warning; endpoint, audit, OpenClaw hook, and observability tests share the FastAPI Safety Service test client and guard against local TestClient construction
Safety Service request helper validation: 38 focused service/observability/request-metadata tests passed, 1 warning; Safety Service unit, observability, and request-metadata tests share canonicalize/classify/observability/default-scope request setup through tests/service_request_helpers.py and guard against direct CanonicalizeRequest/ClassifyRequest imports
