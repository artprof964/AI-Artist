...
Final validation: 380 passed, 1 skipped, 1 warning
...
backend/repo_paths.py: shared repository artifact paths for Compose, env, runbook, OPA policy, and PostgreSQL schema files.
...
definitions are changed.
Repository artifact paths must flow through backend/repo_paths.py before
runtime review checks, scaffold tests, or documentation validators reference
Compose, env, runbook, OPA policy, or PostgreSQL schema files.
