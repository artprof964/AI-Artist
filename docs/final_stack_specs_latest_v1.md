...
Final validation: 383 passed, 1 skipped, 1 warning
...
backend/repo_paths.py: shared repository artifact paths, repo-root resolution, workspace paths/text reads, backend module discovery, and source-text readers for Compose, env, runbook, OPA policy, PostgreSQL schema, and backend module files.
...
composition, env-example rendering, and runtime env resolution must be changed through
...
backend module discovery, and source-text reads must flow through
backend/repo_paths.py before runtime review checks, scaffold tests, contract
guards, or documentation validators reference Compose, env, runbook, OPA
policy, PostgreSQL schema, workspace, or backend module files.
