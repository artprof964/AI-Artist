...
$telegramToken = Get-EnvValue "tele_thestone_04"
if ([string]::IsNullOrWhiteSpace($telegramToken)) {
    throw "Missing required Telegram token env var: tele_thestone_04"
}
...
$env:tele_thestone_04 = $telegramToken
...
$thestone07Token = Get-EnvValue "tele_thestone_07"
if ([string]::IsNullOrWhiteSpace($thestone07Token)) {
    $env:tele_thestone_07 = "__compose_config_unused_thestone_07__"
}
else {
    $env:tele_thestone_07 = $thestone07Token
}

Push-Location $ProjectRoot
try {
    docker compose config --quiet
    docker compose up -d thestone_04-bot
    docker compose ps thestone_04-bot
